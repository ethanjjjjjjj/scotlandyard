...
public class Pathfinding{
    ...
    //calculates the average number of edges mr x is away from the detectives with after the move
    ...
    //Finds the average distance away from the detectives the move produces
    public static int averageMoveDist(Move m,ScotlandYardModel model){
        ArrayList<ScotlandYardPlayer> mutablePlayers=model.getMutablePlayers();
        ArrayList<ScotlandYardPlayer> players = new ArrayList<>(mutablePlayers);
        players.remove(0);
        int distanceSum=0;
        int location;
